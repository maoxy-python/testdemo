...
function goto_class(classstr, key) {
    ...
    if (classstr.indexOf("01") === 0) {
        location.href = "http://search.dangdang.com/book_cover/search_pub.php?key=" + key + "&catalog=" + classstr + "&SearchFromTop=1&ref=suggest-1-1";
        return;
    }
...
function searchsubmit() {
    ...
    if (cnlength(objfrm.key.value) < 2) {
        if (objfrm.key.value.length == 0) {
            if (objfrm.catalog.value > 4000000 && objfrm.catalog.value <= 4000008) {
                ...
                window.location = "http://book_cover.dangdang.com/";
                return false;
            }
    ...
    if (objfrm.catalog.value.indexOf("01") === 0)
        objfrm.action = "http://search.dangdang.com/book_cover/search_pub.php"
